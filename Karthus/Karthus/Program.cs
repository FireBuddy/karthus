                ...
                var location =
                    GetBestCircularFarmLocation(
                        EntityManager.MinionsAndMonsters.EnemyMinions.Where(
                            x =>
                            x.Distance(Player.Instance) <= Q.Range && x.Health > 5 && (x.CountEnemiesInRange(170) == 0) && !x.IsDead && x.IsValid
                            && Prediction.Health.GetPrediction(x, (int)(Q.CastDelay = 1100)) < (1.7 * player.GetSpellDamage(x, SpellSlot.Q)))
                            .Select(xm => xm.ServerPosition.To2D())
                            .ToList(),
                        Q.Width + 5,
                        Q.Range);
                ...
                var location =
                    GetBestCircularFarmLocation(
                        EntityManager.MinionsAndMonsters.EnemyMinions.Where(
                            ...
                            && (Prediction.Health.GetPrediction(x, (int)(Q.CastDelay = 1100)) < 0.9 * player.GetSpellDamage(x, SpellSlot.Q)))
                            .Select(xm => xm.ServerPosition.To2D())
                            .ToList(),
                        Q.Width,
                        Q.Range);
